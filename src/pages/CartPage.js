              ...
              {cart.map((item) => (
                <div key={item.id} className="cart-item">
                  ...
                  <div className="cart-item-info">
                    ...
                    <p className="cart-item-price">💰 Prezzo: <strong>{item.price} €</strong></p>
                    ...
                    {item.cardId ? (
                      <div className="quantity-container">
                        <input 
                          ...
                          onChange={(e) => updateQuantity(item.id, parseInt(e.target.value), item.maxQuantity)}
                        />
                    ...
                    <button className="cart-remove" onClick={() => removeFromCart(item.id)}>❌ Rimuovi</button>
                  </div>
                </div>
              ))}
    
              <div className="cart-summary">
                <h3 className="total-price">💰 Totale: <span>{totalPrice} €</span></h3>
                <button onClick={handleCheckout} className="checkout-button">🛒 Procedi al pagamento</button>
              </div>
            </div>
          ) : (
